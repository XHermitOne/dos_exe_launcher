...
DESCRIPTION = 'DOS Exe File Launcher for Linux by dosbox.'
...
PRG_FILENAME = 'dos_exe_launcher.py'
...
def buildDeb():
    ...
    if os.path.exists(src_prg_filename):
        sysCmd('rm ./deb/usr/bin/*')
        sysCmd('cp %s ./deb/usr/bin/%s' % (src_prg_filename, dst_prg_filename))
        sysCmd('chmod 755 ./deb/usr/bin/%s' % dst_prg_filename)
    else:
    ...
    sysCmd('cp -r ./TOOLS/* ./deb/usr/share/%s' % dst_prg_filename)

    sysCmd('fakeroot dpkg-deb --build deb')

    if os.path.exists('./deb.deb'):
        deb_filename = '%s-%s-%s.%s.deb' % (PACKAGENAME, PACKAGE_VERSION, LINUX_VERSION, LINUX_PLATFORM)
        sysCmd('mv ./deb.deb ./%s' % deb_filename)
    else:
